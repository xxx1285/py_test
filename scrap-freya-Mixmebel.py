...
for category in all_category_in_catalog:
    ...
    response_url_category = requests.get(f'{url_category}?page={num_page})', headers=headers).text
    ...
    with open('1200.csv', 'w', newline='', encoding="utf-8") as file:
        ...
        response = requests.get(f'{link_site_ua})', headers=headers).text
        soup = BeautifulSoup(response, 'lxml')

        all_category_in_catalog = soup.select('.category-wall .caption a[href]')
        ...
        for page in range(1, 3):  # TODO:  number of page (страниц на 1 больше)
            ...
            for link in products_on_page:
                ...
                """ TODO: Name UA """
                name_tovar_ua = soup_link.select_one('.primary_block h1').text
                print(name_tovar_ua)
                ...
                #     '.product-information .tab-content .tab-content-tab-block:nth-child(2) div:first-child span')
                brend = soup_link.select_one('img[src="/img/ico/torgovaya-marka.png"]').next_sibling
                brend = 'Notti' if brend is None else brend.get_text()
                # brend = soup_link.select_one(
                #     '.tab-content #product-features-tab-content .product-desc div:nth-of-type(1)')
                # print(brend)
                # brend = 'Notti' if brend is None else brend.get_text()
                ...
                dlina = soup_link.select_one('img[src="/img/ico/shirina.png"]').next_sibling
                # dlina2 = soup_link.find(attrs={'src': '/img/ico/shirina.png'}).next_element
                dlina = '0' if dlina is None else dlina.get_text()

                """ TODO:  Glubina  """
                glubina = soup_link.select_one('img[src="/img/ico/glubina.png"]')
                print(glubina)
                glubina = '0' if glubina is None else glubina.next_sibling.get_text()
                print(glubina)

                """ TODO:  Visota  """
                visota = soup_link.select_one('img[src="/img/ico/vysota.png"]')
                visota = '0' if visota is None else visota.next_sibling.get_text()

                """ TODO: Spalnoe Mesto  """
                spalnoe = soup_link.select_one('img[src="/img/ico/spalnoe-mesto.png"]')
                if spalnoe is None:
                    spalnoe = '0'
                else:
